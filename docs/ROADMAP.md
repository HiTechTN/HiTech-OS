...
| 1 — OS Core | ✅ Fait, testé (nœud) / 🔧 1er test matériel réel fait, bugs corrigés (ISO) | `configuration.nix` bootable, testé sur `nix build .#...build.vm` réel, SSH fonctionnel. ISO live/installeur (`os/iso.nix` + `os/calamares-hitechos.nix`) : GNOME + Firefox + Calamares (patché) + Flatpak/Flathub + raccourcis menu HiTech-OS. **Premier test réel sur matériel physique (20/09/2026)** : le partitionnement/formatage a fonctionné, mais le job Calamares standard "nixos" (nixos-install) a échoué — `flakes` désactivé dans le nix.conf du système live. Corrigé (`nix.settings.experimental-features` dans `common.nix`). En creusant le log, notre étape "shellprocess@hitechos" n'apparaissait de toute façon pas dans la séquence exécutée : le patch `sed` d'origine ne prenait silencieusement aucun effet dans le bac à sable de build — réécrit sans sed (settings.conf entier généré en Nix, à partir du contenu réel vérifié au commit nixpkgs épinglé dans `flake.lock`). `nix eval` (évaluation complète, sans build) a aussi débusqué un `isoImage.volumeID` trop long et un conflit de précédence sur les mots de passe — corrigés. **Le mécanisme d'installation HiTech-OS lui-même (clone + nixos-rebuild boot) reste à valider au prochain test réel**, celui-ci n'ayant jamais été atteint. `flake.lock` maintenant commité pour la reproductibilité (avant : nixpkgs non épinglé, résolu à chaque build). Mots de passe console (`nixos`/`hitechos`) volontairement faibles, valables uniquement sur cette image live éphémère. |
