...
| 1 — OS Core | ✅ Fait, testé (nœud) / ⚙️ config écrite, non testée (ISO) | `configuration.nix` bootable, testé sur `nix build .#...build.vm` réel, SSH fonctionnel. ISO live/installeur (`os/iso.nix` + `os/calamares-hitechos.nix`) : GNOME + Firefox + Calamares (patché) + Flatpak/Flathub + raccourcis menu HiTech-OS, générée à chaque push par la CI (`iso-build`) et publiée en Release GitHub si < 2 Go (sinon récupérable via l'onglet Artifacts du run). **Calamares déploie désormais automatiquement les services HiTech-OS** (clone du repo + import de `os/common.nix` + `nixos-rebuild boot`, chroté juste après l'install de base) — mécanisme reconstruit à partir du vrai code source de `calamares-nixos-extensions`, mais **jamais testé en conditions réelles** (Calamares ne tourne jamais pendant `nix build`, seule l'ISO elle-même est vérifiée par la CI). À tester en VM avant un disque réel si possible. Mots de passe console (`nixos`/`hitechos`) volontairement faibles, valables uniquement sur cette image live éphémère. |
